...
# Pane ids in live Zellij tab order, as a JSON array of strings (e.g. ["0","2","1",...]).
# Used to sort the per-pane status entries so they match the visible tab order.
pane_order_json() {
    zellij -s "$ZELLIJ_SESSION" action list-panes 2>/dev/null \
        | awk 'NR>1 && $1 ~ /^terminal_/ {sub(/^terminal_/,"",$1); print $1}' \
...
case "$HOOK_EVENT" in
    ...
    SessionEnd)
        ...
        if [ -f "$STATE_FILE" ] && [ -s "$STATE_FILE" ]; then
            SESSIONS=""
            PANE_ORDER=$(pane_order_json); [ -z "$PANE_ORDER" ] && PANE_ORDER='[]'
            while IFS= read -r line; do
                [ -z "$line" ] && continue
                [ -n "$SESSIONS" ] && SESSIONS="${SESSIONS}  "
                SESSIONS="${SESSIONS}${line}"
            done < <(jq -r --argjson order "$PANE_ORDER" '
                to_entries
                | sort_by( (.key) as $k | [ (($order | index($k)) // 9999), ($k | tonumber? // 0) ] )[]
